...
(
    ...
    user_id              varchar(100)               null comment '사용자ID',
    user_password        varchar(300)               null comment '비밀번호',
    user_view_password   varchar(300)               null comment '보이는 비밀번호',
    user_prodvider       varchar(100)               null comment '소셜로그인 공급자 ex)구글, 네이버, 카카오, 애플',
    ...
    user_gender          char                       null comment '회원성별',
    ...
    user_phone_check     char                       null comment '핸드폰 인증 유무',
    ...
    user_authenticated   int default 0              null comment '로그인 가능 유무 토큰유효여부',
    user_status          char                       null comment '회원 상태 ex) 활성, 휴먼, 탈퇴? 이게 왜 있어야 하지?',
    deleted_date         datetime                   null comment '사용자 탈퇴 날짜',
    deleted_flag         int default 0              null comment '사용자 탈퇴여부',
    created_date         datetime default sysdate() null comment '등록일',
    created_id           varchar(15)                null comment '등록자ID',
    modified_date        datetime default sysdate() null comment '수정일',
...
(
    ...
    modified_id    varchar(15)  null comment '수정자ID',
    constraint user_login_history_ibfk_1 foreign key (id) references community.users (id)
) comment '회원 로그인 히스토리';
...
(
    ...
    CREATED_DATE          timestamp    null comment '등록일',
    CREATED_ID            varchar(255) null comment '등록자ID',
    MODIFY_DATE           timestamp    null comment '수정일',
    MODIFY_ID             varchar(255) null comment '수정자ID'
) comment '회원';


create table community.file
(
    id              int auto_increment comment 'ID' primary key,
...
(
    id           int auto_increment comment 'ID' primary key,
    pfi_id       int          default 0  not null comment '첨부파일 테이블 ( post_file table) 의 PK',
    ...
    user_ip      varchar(50)  default '' not null comment '다운로드 IP',
    user_agent   varchar(255) default '' not null comment '다운로드한 유저의 user agent',
    created_date datetime                null comment '등록일',
    created_id   varchar(15)             null comment '등록자ID',
    constraint file_download_log_ibfk_1 foreign key (id) references community.file (id)
) comment '파일 다운로드 로그';
